...
import { monthStartUtcIso } from "../../audit/limits.js";
...
export async function createJob(userId, url, options = {}) {
  const insert = { user_id: userId, url, status: "pending" };

  const initialMeta = {};
  if (options.reportTier) initialMeta.report_tier = options.reportTier;
  if (options.adminRun) initialMeta.admin_run = true;

  if (Object.keys(initialMeta).length > 0) {
    insert.audit_metadata = initialMeta;
  }
  ...
  if (error && insert.audit_metadata) {
    const { audit_metadata: _omit, ...fallbackInsert } = insert;
...
export async function countUserAuditsThisMonth(userId) {
