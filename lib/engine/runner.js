...
export async function runAudit(jobId, url, _userId) {
  try {
    const llm = getLlmConfig();
    console.log(
      `[runAudit] LLM ${llm.provider} (primary: ${llm.models.primary}, fast: ${llm.models.fast})`
    );
    ...
    if (pipeline) {
      ...
      allIssues = await pipeline.finish();
    } else {
      await heartbeat(jobId, "analyzing");
      allIssues = await runAnalysisOnAllSections(
        desktopSections,
        mobileSections,
        siteType,
        jobId
