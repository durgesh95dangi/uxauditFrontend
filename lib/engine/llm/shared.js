...
function normalizeAuditPayload(parsed) {
  ...
  return {
    ...parsed,
    section_score:
      parsed.section_score ??
      parsed.sectionScore ??
      parsed.score ??
      null,
    above_fold: parsed.above_fold ?? parsed.aboveFold ?? null,
    issues
  };
}
...
export function parseSectionAuditJson(rawText) {
  ...
  try {
    const parsed = normalizeAuditPayload(JSON.parse(cleaned));
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      return parsed;
    }
  } catch {
  ...
  try {
    const parsed = normalizeAuditPayload(JSON.parse(match[0]));
