...
import { isVertexConfigured } from "../../vertex/client.js";
...
function readGeminiUseVertex() {
  const raw = process.env.GEMINI_USE_VERTEX;
...
function readApiKey(provider) {
  ...
  if (provider === "gemini") {
    if (readGeminiUseVertex() && isVertexConfigured()) {
      return "";
    }
...
export function getLlmConfig() {
  ...
  const useVertex = provider === "gemini" && readGeminiUseVertex() && isVertexConfigured();
