...
import { REPORT_TIER_FULL, resolveJobReportTier } from "../../audit/reportTier.js";
...
export async function resumePendingJobs() {
  const { data, error } = await supabase
    .from("audit_jobs")
    .select("id, url, user_id, audit_metadata")
    .eq("status", "pending");
  ...
  for (const job of data) {
    ...
    const adminRun = job.audit_metadata?.admin_run === true;

    enqueue(() =>
      runAudit(job.id, job.url, job.user_id, { reportTier, adminRun })
    ).catch((err) => {
