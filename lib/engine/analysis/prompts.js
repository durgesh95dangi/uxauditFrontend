// prompts.js - centralizes all LLM prompts used across the audit analysis pipeline
...
F5. Are there blank spaces or placeholders suggesting content that loaded late or failed?`,
  ...
  navigation: `
AREA: navigation — SITE NAV / HEADER BAR ONLY
N1. Are primary nav links labeled clearly (not vague labels like "Solutions" with no context)?
N2. Is the nav hierarchy scannable — most important destinations easy to find?
N3. If a nav CTA button exists — is its label specific and visually distinct?
N4. On mobile — is the menu pattern obvious and usable?
N5. Are tap targets in the nav adequately sized and spaced?

DO NOT apply hero or landing-page checks here:
- Do NOT flag missing value proposition, product explanation, headline, subheadline, or "what the company does".
- Do NOT expect marketing copy, benefits, or proof points in the nav bar.
- Those belong in the hero/main content section below the header.`
};

const PROFILE_GUIDANCE = {
  nav: `
SECTION CONTEXT: This screenshot is the site header or navigation bar (top chrome), NOT the page hero.
Never report missing value proposition, unclear offering, absent headline/subheadline, or "visitor cannot tell what the company does" — that is incorrect for navigation.
Only flag issues visible in the nav itself (labels, hierarchy, accessibility, nav CTA if present).`
};
...
export function buildSectionAuditText(sectionLabel, viewport, options = {}) {
  ...
  const profileGuidance = PROFILE_GUIDANCE[profile] || "";
