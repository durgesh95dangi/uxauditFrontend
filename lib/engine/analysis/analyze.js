...
export async function analyzeSection(
  ...
  try {
    ...
    return { ...EMPTY_SECTION_RESULT, llmError: message };
  }

  if (!text?.trim()) {
    const message = `${providerName} returned an empty analysis response`;
    console.error(`[analyzeSection] ${message} for ${sectionLabel} (${viewport})`);
    return { ...EMPTY_SECTION_RESULT, llmError: message };
  }
  ...
  if (parsed === null) {
    console.error(
      `[analyzeSection] ${providerName} returned invalid JSON for ${sectionLabel} (${viewport}):`,
      text.slice(0, 500)
    );
    return {
      ...EMPTY_SECTION_RESULT,
      llmError: "Model returned invalid JSON",
