...
function normalizeSectionResult(parsed, profile = "default") {
...
export async function analyzeSection(
  ...
  console.log(
    `[analyzeSection] ${sectionLabel} (${profile}, ${model}): score ${result.sectionScore ?? "—"}/100, ${result.issues.length} issue(s), ~${inputTokens} in tokens`
  );
